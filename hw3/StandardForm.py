...
def standard_form(input_file_name, output_file_name):
    with open(input_file_name, 'r') as rfile, open(output_file_name, 'w') as wfile:
        ...
        wfile.write('{0} {1}\n'.format(n, k))
        for i in range(k):
            line = ''.join([str(matrix[i, j]) for j in range(n)])
            wfile.write(line + '\n')

        indexes = [p + 1 for p in np.argsort(positions)]
        wfile.write(' '.join([str(i) for i in indexes]))


def main():
    input_file_name, output_file_name = get_parameters()
    standard_form(input_file_name, output_file_name)
