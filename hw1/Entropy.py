...
def calculate_and_store_distribution(input_file_name, output_file_name):
    with open(input_file_name, 'r', encoding='UTF-8') as rfile, open(output_file_name, 'w', encoding='UTF-8') as wfile:
        ...
        # prefix_count = COL.defaultdict(lambda:0)
        ...
        for character in characters:
            ...
            for suffix in characters:
                ...
                # prefixed_prob = dual_dict[character + suffix] / prefix_count[character] if prefix_count[character] != 0 else 0
                prefixed_prob = dual_dict[character + suffix] / single_dict[character] if single_dict[character] != 0 else 0
                prefixed_entropy += single_prob * prefixed_prob * (-math.log(prefixed_prob, 2)) if prefixed_prob != 0 else 0

        wfile.writelines([str("%.7f" % symnbol_entropy) + '\n', str("%.7f" % dual_entropy) + '\n',  str("%.7f" % prefixed_entropy)])
